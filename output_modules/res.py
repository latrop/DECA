...
try:
	...
	print "fpdf module has not yet been installed!"
tmp_out = sys.stdout
...
def results(galaxy_name,model,out_image,file_out_txt,file_out_pdf,filter_name,colour_name,colour_value,Aext,z,pix2sec,m0,D,status,tables='tog',gal_number=0,out=1,ima_number=0):
	if out_image!='0':
	...
	if model=="edge" or model=="edge+ser":
			m0_d = map(float, re.findall(r"[-+]?\d*\.\d+|\d+", prihdr['1_MU_0']))
			z0 =  map(float, re.findall(r"[-+]?\d*\.\d+|\d+", prihdr['1_HS']))
			h_d = map(float, re.findall(r"[-+]?\d*\.\d+|\d+", prihdr['1_RS']))
			PA_d = map(float, re.findall(r"[-+]?\d*\.\d+|\d+", prihdr['1_PA']))
			xc_d = map(float, re.findall(r"[-+]?\d*\.\d+|\d+", prihdr['1_XC']))
			yc_d = map(float, re.findall(r"[-+]?\d*\.\d+|\d+", prihdr['1_YC']))
			if '1_C0' in prihdr:
				...
				c_d = [-1.,0.]
			try:
				rtr = map(float, re.findall(r"[-+]?\d*\.\d+|\d+", prihdr['2_RBRK']))
				rsoft = map(float, re.findall(r"[-+]?\d*\.\d+|\d+", prihdr['2_DSFT']))
			except:
				print 'There is no truncation!'
			...
			try:
				rtr[0] = rtr[0]*pix2sec
				rtr[1] = rtr[1]*pix2sec  
				rsoft[0] = rsoft[0]*pix2sec 
				rsoft[1] = rsoft[1]*pix2sec 
			except:
				print 'There is no truncation!'
			magDisk = magDisk_edge_on(m0_d[0],h_d[0],z0[0],c_d[0])
			lumDisk = 10**(0.4*(m0-magDisk))
			...
			if z!='NO':
				MagDisk = MagDisk_edge_on(m0_d[0],h_d[0],z0[0],c_d[0],z,filter_name,colour_name,colour_value,Aext)
				LumDisk = Luminosity(MagDisk,filter_name)
				h_d_k = scale(h_d[0],D)
				h_d_k_e = scale(h_d[1],D)
				z_0_k = [scale(z0[0],D),scale(z0[1],D)]
	...
	if model=="edge+ser":
		...
		try:
			me_bul = map(float, re.findall(r"[-+]?\d*\.\d+|\d+", prihdr['3_MU_E']))
			re_bul = map(float, re.findall(r"[-+]?\d*\.\d+|\d+", prihdr['3_RE']))
			n_bul = map(float, re.findall(r"[-+]?\d*\.\d+|\d+", prihdr['3_N']))
			PA_bul = map(float, re.findall(r"[-+]?\d*\.\d+|\d+", prihdr['3_PA']))
			if setup.fix_ell==1:
				q_bul = map(float, re.findall(r"[-+]?\d*\.\d+|\d+", prihdr['3_AR']))
			else:
				q_bul = [1.,0.]
			xc_bul = map(float, re.findall(r"[-+]?\d*\.\d+|\d+", prihdr['3_XC']))
			yc_bul = map(float, re.findall(r"[-+]?\d*\.\d+|\d+", prihdr['3_YC']))
			if '3_C0' in prihdr:
				c_bul = map(float, re.findall(r"[-+]?\d*\.\d+|\d+", prihdr['3_C0']))
			else:
				c_bul = [0.,0.]
		except:
	...
	if model=="ser":
		...
		def MagBulge_f(reb,meb,n,q,c,z,filter_name,colour_name,colour_value,Aext):
			return magBulge_f(reb,meb,n,q,c) - 5.*log10(D_L(z)) - Aext - cosmo.A_kcor(filter_name, z, colour_name, colour_value) - 25.
		me_bul = map(float, re.findall(r"[-+]?\d*\.\d+|\d+", prihdr['1_MU_E']))
		re_bul = map(float, re.findall(r"[-+]?\d*\.\d+|\d+", prihdr['1_RE']))
		n_bul = map(float, re.findall(r"[-+]?\d*\.\d+|\d+", prihdr['1_N']))
		PA_bul = map(float, re.findall(r"[-+]?\d*\.\d+|\d+", prihdr['1_PA']))
		if setup.fix_ell==1:
			q_bul = map(float, re.findall(r"[-+]?\d*\.\d+|\d+", prihdr['1_AR']))
		else:
			q_bul = [1.,0.]
		xc_bul = map(float, re.findall(r"[-+]?\d*\.\d+|\d+", prihdr['1_XC']))
		yc_bul = map(float, re.findall(r"[-+]?\d*\.\d+|\d+", prihdr['1_YC']))
		if '1_C0' in prihdr:
			c_bul = map(float, re.findall(r"[-+]?\d*\.\d+|\d+", prihdr['1_C0']))
		else:
			c_bul = [0.,0.]
		re_bul[0] = pix2sec*re_bul[0]
		re_bul[1] = pix2sec*re_bul[1]
	...
	if out==1:
		return xc_d[0],yc_d[0],m0_d[0],h_d[0],q_d[0],z0[0],rtr[0],xc_bul[0],yc_bul[0],me_bul[0],re_bul[0],q_bul[0],n_bul[0],c_bul[0],chi2
	...
	if (file_out_pdf!='NONE' or file_out_pdf!='NO') and status==0:
		...
		if model=='edge' or model=='edge+ser':
			if jpeg==0:
	 	 		pdf.image('galaxy.jpeg',x=5,y=51.,h=dimy,w=dimx)
		  		pdf.image('model.jpeg',x=75,y=51.,h=dimy,w=dimx)
				pdf.image('residual.jpeg',x=145,y=51.,h=dimy,w=dimx)
			else:
