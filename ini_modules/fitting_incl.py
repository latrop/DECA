...
def cross_radius(m0d_ini,h_ini,meb_ini,reb_ini,n_ini):
		import scipy.optimize as optimize

		def Icross(r): 
			nu = 1.9992*n_ini - 0.3271
			return meb_ini - 2.5*nu/log(10.) + 2.5*nu/log(10.) * (fabs(r)/reb_ini)**(1.0/n_ini) - m0d_ini - 1.0857*fabs(r)/h_ini

		i = 0
		rr = 0
		cross1 = 1000.
		while i< 1 and rr<1000.:
			rr = rr + 1
			try:
				cross1 = optimize.bisect(Icross,0,rr)
				i = 1
			except:
				i = 0
			
		#print 'cross1=%.3f' % (cross1)
		return cross1
	...
	if model=='exp+ser':
		...
		if C31>Ccr:
			...
			if BT_c31(C31)>0.:
				...
				magBul0 = mag_auto - 2.5*log10(0.5)
				magDisk0 = mag_auto -2.5*log10(1.-0.5)			
		else:
			n0 = 1.5
			if BT_c31(C31)>0.:
				magBul0 = mag_auto - 2.5*log10(BT_c31(C31))
				...
				magBul0 = mag_auto - 2.5*log10(0.1)
				magDisk0 = mag_auto -2.5*log10(1.-0.1)
...
def  main(ff_log,number,xc,yc,kron_r,flux_radius,mag_auto,a_image,b_image,PA,NOISE,pix2sec,m0,FWHM,EXPTIME,ini_use,find_model,model,ellip,C31,sky_level,file_out_txt,file_out_pdf,filter_name,colour_name,colour_value,Aext,z,D,galaxy_name,find_psf,rmax,mode):
	...
	while (status!=0 and k<len(model) and n_first==0)  or (k<2):
		...
		if ini_use=='NO':
			#print 'pars',xc,yc,PA,m0,pix2sec,NOISE,a_image*kron_r,FWHM,Model,ellip,C31
			if way==2:
				try:
					...
					if Model=='exp' or Model=='exp+ser' or Model=='ser':
						m0d0,magDisk0,h0,z00,elld0,cd0,rtr0,meb0,magBul0,reb0,n0,ellb0,cb0 = INITIALS.ini_prof('azim',xc,yc,PA,m0,pix2sec,NOISE,a_image*kron_r,FWHM,Model,ellip,C31,ff_log)
						good = 0
						if math.isnan(float(m0d0))==True or n0<0. or math.isnan(float(meb0))==True or reb0<0.:
							good=1
				...
				#exit()
			if way==3 or good!=0 or h0<reb0 or h0>4.*reb0 or reb0<1.:
				try:
					print 'Way to find initial parameters: 3'
					print >>ff_log, 'Way to find initial parameters: 3'
					if Model=='exp' or Model=='exp+ser' or Model=='ser':
						m0d0,magDisk0,h0,z00,elld0,cd0,rtr0,meb0,magBul0,reb0,n0,ellb0,cb0 = INITIALS.ini_prof('cut',xc,yc,PA,m0,pix2sec,NOISE,a_image*kron_r,FWHM,Model,ellip,C31,ff_log)
						good = 0
					elif Model=='edge' or Model=='edge+ser':
			...
			if way==4 or good!=0 or h0<reb0 or h0>4.*reb0 or reb0<1.:
				...
				good = 0
				if reb0<1. or n0<0.5:
					print 'Initial parameters are not found!'
		...
		if (Model=='exp' or Model=='ser' or Model=='exp+ser' or change_rmax==1) and lim_pars==2:
			#print cross,rmax
			#exit()
			constraints.constr(file_constraints,Model,0,FWHM,pix2sec,h0,rmax,fix=1)	# Creating constraints file
			if BT0>=0.7:	m0d0 = m0d0# - 0.5
		elif (Model=='edge' or Model=='edge+ser' or change_rmax!=1) and lim_pars==2:
			constraints.constr(file_constraints,Model,1,FWHM,pix2sec,h0,rmax,fix=1)	# Creating constraints file
			if cross>=rmax/2.:	m0d0 = m0d0# - 1.0
		...
		cross = cross_radius(m0_d,h*pix2sec,me_bul,re_bul*pix2sec,n_bul)
		if cross>rmax*pix2sec/2. and lim_pars==0:
			lim_pars = 0
		else:
			lim_pars = 1

		if status==0 and BT0>0.06 and rad_ratio0>1.3 and BT0<0.75 and lim_pars!=0:
			k = 4 # Exit from the cicle

		if status==0 and BT0>=0.75 and nbul0>1.5 and lim_pars!=0:
			k = 4

		if status==0 and ((BT0>=0. and BT0<=0.06) or rad_ratio0<1.3 or (BT0>=0.75 and nbul0>0.93 and nbul0<1.07) or k_iter>3) and lim_pars!=0:
			Model = 'exp'
			k = k + 1
			if k==2: 	k = 3 # Exit from the cicle

		if status==0 and out_nan==1 and lim_pars!=0:
			out_nan=0
			Model = model[1]
			k = k + 1

		if status==0 and Model=='exp' and k_iter>2 and lim_pars!=0:
			k = 4


		if status!=0 and k_iter>3 and lim_pars!=0:
			k = 4


		if status!=0 and change_rmax==1 and k_iter!=1 and lim_pars!=0:
			Model = model[1]
			k = k + 1
			if k==2: 	k = 5 # Exit from the cicle

		if status!=0 and ini_use=='YES' and lim_pars!=0:
			ini_use='NO'

		if status==0 and cross>rmax*pix2sec/2. and lim_pars==0 and k_iter<=3:
			Model = model[0]
			lim_pars = 2
	...
	if n_it>1:
		...
		if setup.inner_trunc==1:
			if kk==0 and rtr[kk]<2.*h[kk] and chi2[1]<2. and Model[1]=='exp+ser':
				kk = 1
				#print 'Ku Ku!'
		...
		try:
			for p in range(len(xc_d)):
				if setup.lim_pars==0 and cross_radius(m0_d[kk],h[kk]*pix2sec,me_bul[kk],re_bul[kk]*pix2sec,n_bul[kk])>=2*rmax*pix2sec/3. and Model[p]=='exp+ser' and (chi2[p]<1.5 or chi2[p]>4.): 
					kk = p
					#print "I am here 1!"
		except:
			p = 1
		...
		if (k==3 or n_bul[kk]>6. or h[kk]/re_bul[kk]<1.3) and chi2[1]<=1.0:			#!!!!!!!!!
			try:
				kkk = Model.index('exp')
...
def second_iter(number,Model,k_iter,nx,ny,xc,yc,pix2sec,m0,EXPTIME,sky_level,file_out_txt,file_out_pdf,filter_name,colour_name,colour_value,Aext,z,D,galaxy_name,find_psf,m0d0,h0,z00,elld0,rmax,meb0,reb0,n0,ellb0,cb0,chi2,FWHM):
		...
		print 'Inner (bar) radius: %.2f' % (optimalInnerRadius)
		print 'Outer radius: %.2f' % (diskR25)
		print 'Total angular rotation out to outer radius: %.2f' % (angularRotation)
		print 'Inclination angle to line of sight: %.2f' % (galaxyIncl)
		print 'Position angle in the plane of the sky: %.2f' % (PAd)
		print '*************************************************************\n'
		change_rmax=1

		if Model=='exp' or Model=='ser' or Model=='exp+ser' or change_rmax==1:
			constraints.constr1(file_constraints,Model,0,FWHM,pix2sec,h0,optimalInnerRadius,diskR25,angularRotation,galaxyIncl)
		else:
			constraints.constr1(file_constraints,Model,1,FWHM,pix2sec,h0,optimalInnerRadius,diskR25,angularRotation,galaxyIncl)
		...
		if (Model=='ser' or Model=='exp+ser' or change_rmax==1):
			constraints.constr1(file_constraints,Model,0,FWHM,pix2sec,h0,r_in,r_out,r_angle,incl_angle,R10=spa,n_iter=3)
		else:
			constraints.constr1(file_constraints,Model,1,FWHM,pix2sec,h0,r_in,r_out,r_angle,incl_angle)
		...
		if status==0 and (Model=='ser' or Model=='exp+ser'):
			xc_d,yc_d,m0_d,h_d,r_in,r_out,r_angle,incl_angle,spa,pa,rtr,xc_bul,yc_bul,me_bul,re_bul,q_bul,n_bul,c_bul,chi2 = res_spiral.results(galaxy_name,Model,'0',file_out_txt,file_out_pdf,filter_name,colour_name,colour_value,Aext,z,pix2sec,m0,D,status,tables='tog',gal_number=0,out=1,fix=1)
			shutil.move(file_galfit_outimage,'./pics/%i/out_%i.fits' % (number,k_iter+1))
			shutil.move('subcomps.fits','./pics/%i/sub_%i.fits' % (number,k_iter+1))
			os.remove('galfit.01')
